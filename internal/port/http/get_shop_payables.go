...
func getShopPayables(_ context.Context, iUseCases interfaces.IUseCases) server.MiddleFunc {
	return func(ctx context.Context, r server.RawRequest) (interface{}, error) {
		...
		if !ok || spID == nil || spID[0] == "" {
			return nil, fmt.Errorf("deleteCardHandler >>  %w", models.ErrIDIsNotValid)
		}
		...
		response, err := iUseCases.GetShopPayables(tokenCTX, &request)
		if err != nil {
