...
func updatePayablePartly(_ context.Context, iUseCases interfaces.IUseCases) server.MiddleFunc {
	return func(ctx context.Context, r server.RawRequest) (interface{}, error) {
		...
		response, err := iUseCases.UpdatePayablePartly(tokenCTX, &request)
		if err != nil {
			return nil, fmt.Errorf("updateShopReceivable >> %w", err)
		}
