...
func (m *Mysql) GetBuyFactorByNumber(ctx context.Context, factorNumber string) (*dtobuy.Response, error) {
	...
	eStmt, err := m.db.PrepareContext(ctx, "SELECT p.id, amount, status, clear_date, p.created_at, p.updated_at "+
		"FROM shop_payable p INNER JOIN factors ON p.factor_id = factors.id "+
		"WHERE p.factor_id = ?")
	if err != nil {
		return nil, fmt.Errorf("mysql >> GetFactorByNumber >> PrepareContext() >> %w", err)
	}
	...
	response.Equities = make([]dtobuy.Equity, 0)

	for r.Next() {
		var equity dtobuy.Equity
