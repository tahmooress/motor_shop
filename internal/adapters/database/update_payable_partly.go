...
func (m *Mysql) UpdatePayablePartly(ctx context.Context, equity models.ShopEquity,
	...
	stmt, err := tx.PrepareContext(ctx, "Update shop_payable SET status = ?, amount = ? WHERE id = ?")
	if err != nil {
		return nil, err
	}
	...
	newEquity, err := tx.PrepareContext(ctx, "INSERT INTO shop_payable(id, customer_id, factor_number,"+
	...
	if err != nil {
		return nil, fmt.Errorf("mysql >> UpdatePayablePartly >> PrepareContext() >> %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("mysql >> UpdatePayablePartly >> ExecContext() >> %w", err)
	}
