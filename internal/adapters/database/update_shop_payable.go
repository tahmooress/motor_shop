...
func (m *Mysql) UpdateShopPayable(ctx context.Context, equityID models.ID) (*models.ShopEquity, error) {
	var (
		factorNumber string
		ShopID       string
		amount       float64
	...
	fStmt, err := m.db.PrepareContext(ctx, "SELECT shop_id, factor_number, amount FROM shop_payable WHERE id = ?")
	if err != nil {
	...
	stmt, err := tx.PrepareContext(ctx, "UPDATE shop_payable SET status = ? WHERE id = ?")
	if err != nil {
		return nil, fmt.Errorf("mysql >> UpdateShopReceivable >> PrepareContext() >> %w", err)
	}
